...
class Ui(QtWidgets.QMainWindow):
    ...
    def frame_processing(self, faces, img):
        for (x,y,w,h) in faces:                                                               
            cv2.rectangle(self.img,(x,y),(x+w,y+h),(36,255,12),4)
            self.update_picture_label_2(self.img)
    ...
    def getFaceROI(self,img):
        # global models_init
        gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        faces = self.models_init.faceCascade.detectMultiScale(gray, 1.2, 5)
        if len(faces)>0:
            img=cv2.rectangle(img, (faces[0][0] + int(self.x1*faces[0][2]), faces[0][1]+ int(self.y1*faces[0][3]) ), (faces[0][0]+ int(self.x2*faces[0][2]), faces[0][1]+ int(self.y2*faces[0][3]) ), (255, 0, 0), 2)
            return [faces[0][0]+int(self.x1*faces[0][2]),  faces[0][1]+ int(self.y1*faces[0][3]),  faces[0][0]+ int(self.x2*faces[0][2]), faces[0][1]+ int(self.y2*faces[0][3])]
        else:
            return [0,0,0,0]
        
    def getColorSum(self, frame, color_id):
        return frame[:,:,color_id].sum()

    def getColorAverage(self, frame, color_id):
        return frame[:,:,color_id].sum()*0.80 / (frame.shape[0]*frame.shape[1])
    
    def main_puls(self,frame):
        self.x1 = 0.4
        self.x2 = 0.6
        self.y1 = 0.1
        self.y2 = 0.25
        fps = self.models_init.cap.get(cv2.CAP_PROP_FPS)
        self.step = int(1000/fps)


        global previous_frame
        if (self.idf==0):
            droi = self.getFaceROI(frame)
            if (droi[3] > 0) :
                self.bbox = droi
        if(self.idf > 0):
        ...
        self.img=cv2.rectangle(self.img, (self.bbox[0] , self.bbox[1] ), ( self.bbox[2],  self.bbox[3] ), (255, 0, 0), 2)
        green = self.getColorAverage(roi, 1)  ## 2nd channel for Green color
        previous_frame = self.img
        print(green)
        self.gsums.append(green)
        self.plot_graph(self.gsums)
